...
def main(args):
    ...
    out.save(os.path.join("result/", "out_{}".format(img_name)))

    print("Saving the output...")
    raw_out = TF.to_pil_image(raw_out)
    img_name = args.img.split('/')[-1]
    raw_out.save(os.path.join("result/", "ori_{}".format(img_name)))


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Specify the inputs")
    parser.add_argument('--img', type=str, default="E:/graduation/project/data/DRIVE_interest/test/images/12_test.tif")
    parser.add_argument('--mask', type=str, default="E:/graduation/project/data/DRIVE_interest/test/1st_manual/12_manual1.gif")
